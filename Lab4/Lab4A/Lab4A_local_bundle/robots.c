...
void turn_left(Robot *r){             //row68, change the direction of robot when it turns left
    r->dir = LEFT_OF[r->dir];        //r is a pointer point to the robot struct,which can change the value of the direction of the robot
                                    //if we do not use pointer,we only change the copy of the robot struct in the function stack
}  //r->dir is a direction and the direction also means a number which was defined in the enum Direction.Get the new direction by viewing the LEFT_OF array at the index r->dir
...
void forward_delta(Direction d, int *dx, int *dy){
    *dx = DX[d];        //the function is uesd in the step_forward function to simplify the process of calculating the new position according to the direction
    *dy = DY[d];  //store the change of x and y of differdnt direction and get the change by viewing the DX and DY array at the index d
}
...
int find_robot(Robot *rs, int m, const char *name){  //row65,search the robot so the next step can be executed on the right robot
    for (int i = 0; i < m; i++){      //rs is a pointer point of the struct Robot array,using for loop to check all the robots inside
        if (strcmp(rs[i].name, name) == 0){  //the function strcmp:compare two strings,if they are the same,return 0,otherwise return a non-zero value
